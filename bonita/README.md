...
```console
$ docker run --name bonita_postgres --link mydbpostgres:postgres -d -p 8080:8080 arm64v8/bonita
```
...
```console
$ docker run --name bonita_mysql --link mydbmysql:mysql -d -p 8080:8080 arm64v8/bonita
```

## Modify default credentials

```console
$ docker run --name=bonita -e "TENANT_LOGIN=tech_user" -e "TENANT_PASSWORD=secret" -e "PLATFORM_LOGIN=pfadmin" -e "PLATFORM_PASSWORD=pfsecret" -d -p 8080:8080 arm64v8/bonita
```
...
2.	Start your `bonita` container like this:
	...
	docker run --name some-bonita -v /my/own/datadir:/opt/bonita -d -p 8080:8080 arm64v8/bonita:tag
...
-	Launch the new container pointing towards the copy of DB and filesystem
	...
	```console
	$ docker run --name=bonita_postgres --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -v "$PWD"/bonita_migration:/opt/bonita/ -d -p 8081:8080 arm64v8/bonita:7.2.4
	```

	-	If >= 7.3.0

	```console
	$ docker run --name=bonita_postgres --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -d -p 8081:8080 arm64v8/bonita:2021.2-u0
	```
...
```console
$ docker run  -e HTTP_API=true -e REST_API_DYN_AUTH_CHECKS=false --name bonita -d -p 8080:8080 arm64v8/bonita
```
...
$ docker run --name bonita_custom -v "$PWD"/custom_bonita/:/opt/custom-init.d -d -p 8080:8080 arm64v8/bonita
```
