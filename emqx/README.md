...
**Note:** this is the "per-architecture" repository for the `ppc64le` builds of [the `emqx` official image](https://hub.docker.com/_/emqx) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![ppc64le/emqx build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/emqx.svg?label=ppc64le/emqx%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/emqx/)
...
```console
$ docker run -d --name emqx ppc64le/emqx:tag
```

For example

```console
$ docker run -d --name emqx -p 18083:18083 -p 1883:1883 ppc64le/emqx:latest
```
...
```console
$ docker run -d --name emqx -e EMQX__LISTENERS__TCP__DEFAULT__BIND=1883 -p 18083:18083 -p 1883:1883 ppc64le/emqx:latest
```
