...
**Note:** this is the "per-architecture" repository for the `amd64` builds of [the `rocket.chat` official image](https://hub.docker.com/_/rocket.chat) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
-	[`4.6.1`, `4.6`, `4`, `latest`](https://github.com/RocketChat/Docker.Official.Image/blob/6c3b54a4ea76d6e838c70b876acf92893808ad54/4.6/Dockerfile)
...
```console
$ docker run --name rocketchat --link db --env MONGO_OPLOG_URL=mongodb://db:27017/local -d amd64/rocket.chat
```
...
```console
$ docker run --name rocketchat -p 80:3000 --link db --env ROOT_URL=http://localhost --env MONGO_OPLOG_URL=mongodb://db:27017/local -d amd64/rocket.chat
```
...
```console
$ docker run --name rocketchat -p 80:3000 --env ROOT_URL=http://localhost --env MONGO_URL=mongodb://mymongourl/mydb --env MONGO_OPLOG_URL=mongodb://mymongourl:27017/local -d amd64/rocket.chat
```
